...
/*
다음 순열을 구하는 알고리즘을 익히기 위해 직접 짜본 코드
*/

bool my_next_permutation(int* input_array, int array_size) {
	// input_array를 다음 순열로 바꿔 주고, 그렇게 하지 못할 경우에는 false를 출력해주는 함수

	// 1. input_array의 뒤에서부터 살펴보면서, 뒤에서부터 가장 길게 내림차순인 부분을 찾는다. 723"6541" 이런 array가 주어진다고 하면, 6541부분은 가장 긴, 끝에서부터 연결된 내림차순이니까 723으로 시작하는 마지막 수열임을 알 수가 있다.
	// 만약 여기서 모든 수열이 987654와 같이 내림차순이면, 다음 순열은 없으므로 false
	int end = 0;
	...
	// 2. 주어진 순열의 다음 순열은 72?로 시작하는 첫 순열, 이 ?자리는 뒤에 있는 내림차순 부분에서 3보다 큰 수 중 가장 작은 수가 들어가야 하므로 탐색하면서 구한 뒤 3과 위치를 바꿔준다.
	int index = end;
